                      ...
                      <div className="flex gap-1.5 flex-wrap">
                        {([
                          ...
                          <button key={key} onClick={() => { setTab(key); setPage(0); }}
                            ...
                            {label}
                            <span className={['text-xs px-1.5 py-0.5 rounded-full font-bold', tab === key ? 'bg-white/20 text-slate-900 dark:text-white' : 'bg-slate-100 text-on-surface-variant'].join(' ')}>{count}</span>
                          </button>
                        ))}
                      </div>
