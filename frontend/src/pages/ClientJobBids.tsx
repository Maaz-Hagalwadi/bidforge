          ...
          <div className="flex-1 p-6 pb-8 lg:p-8 max-w-[1280px] w-full mx-auto space-y-6">
            ...
            {loading ? (
              ...
              <div className="tonal-card rounded-xl flex flex-col items-center gap-4 py-20 text-center">
                <span className="material-symbols-outlined text-5xl text-slate-600 dark:text-slate-300">error_outline</span>
              ...
              <>
                ...
                {bids.length === 0 ? (
                  <div className="tonal-card rounded-xl flex flex-col items-center gap-4 py-20 text-center">
                    <span className="material-symbols-outlined text-5xl text-slate-600 dark:text-slate-300">gavel</span>
                    ...
                    <div className="flex gap-1.5 flex-wrap">
                      {([
                        ...
                        <button key={key}
                          ...
                          {label}
                          <span className={['text-xs px-1.5 py-0.5 rounded-full font-bold', statusTab === key ? 'bg-white/20 text-slate-900 dark:text-white' : 'bg-slate-100 text-on-surface-variant'].join(' ')}>{count}</span>
                        </button>
                      ))}
                    </div>
                            ...
                            return (
                              <article key={bid.id} className="tonal-card rounded-xl overflow-hidden hover:shadow-md transition-all">
                                <div className="p-5 flex flex-col md:flex-row md:items-start gap-4">
                                  {/* Freelancer avatar */}
                                  <div className="w-10 h-10 rounded-full bg-secondary flex items-center justify-center text-slate-900 dark:text-white text-sm font-bold flex-shrink-0">
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 space-y-5" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                disabled={actioning}
                className={`flex-1 py-2.5 text-slate-900 dark:text-white text-sm font-semibold rounded-lg hover:brightness-110 active:scale-[0.98] transition-all disabled:opacity-60 ${confirmAction.type === 'accept' ? 'bg-secondary' : 'bg-red-500'}`}
              >
                {actioning
