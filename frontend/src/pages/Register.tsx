...
export default function Register() {
  ...
  const handleGoogleSuccess = async (accessToken: string) => {
    ...
    setGoogleError(null);
    try {
      const loggedInUser = await loginWithGoogle(accessToken, selectedRole as 'CLIENT' | 'FREELANCER');
      navigate(loggedInUser.role === 'CLIENT' ? '/client/dashboard' : '/freelancer/dashboard', { replace: true });
    } catch (err) {
  ...
  return (
    <div className="bg-dark-navy antialiased flex flex-col h-screen overflow-hidden">
      {(isSubmitting || googleLoading) && <BidForgeLoader message={googleLoading ? 'Signing in with Google…' : 'Creating your account…'} />}
      <Navbar variant="auth" />
      ...
      <main className="flex-1 flex flex-col md:flex-row overflow-hidden min-h-0">
        ...
        <section className="w-full md:w-1/2 bg-dark-navy flex items-center justify-center px-8 md:px-14 overflow-y-auto">
          <div className="w-full max-w-md py-4">
            ...
            <div className="rounded-xl border border-slate-700/60 bg-slate-900/40 p-md flex items-center gap-sm">
              <span className="material-symbols-outlined text-secondary text-base">
                {selectedRole === 'CLIENT' ? 'person_search' : 'work'}
              </span>
              <p className="text-slate-400 text-body-sm flex-1">
                Signing up as <span className="text-white font-medium">{selectedRole === 'CLIENT' ? 'a Client' : 'a Freelancer'}</span>
              </p>
            </div>
